...
jmp1:write('     Enter number of grids   (max:50):');
    ...
    readln(filename);
    if (filename='') then filename:='blank.fon';
   init_g;
   ...
   outtextxy(305,226,'  S:save,I:inverse             ');
   x:=1;
   y:=1;
...
until endchk;
   ...
   close(infile);
      ...
      writeln('Do you want to save this file with another name(Y/N)?');
   readln(answer);
   if ((answer='Y') or (answer='y')) then
      begin
         writeln('please type the new filename..:');
         read(filename);
      end;
   assign(infile,filename);
   rewrite(infile);
